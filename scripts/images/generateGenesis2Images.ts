...
async function main() {
  ...
  // 4. 결과 저장
  const resultPath = path.join(__dirname, 'genesis2_image_results.json');
  fs.writeFileSync(resultPath, JSON.stringify({
    chapter: 2,
    totalWords: words.length,
    generated: results.filter(r => r.imageUrl).length,
    failed: results.filter(r => !r.imageUrl).length,
    results
  }, null, 2), 'utf-8');

  log.step('완료');
  log.success(`생성 완료: ${results.filter(r => r.imageUrl).length}개`);
  log.error(`실패: ${results.filter(r => !r.imageUrl).length}개`);
  log.info(`결과: ${resultPath}`);

  log.step('💡 다음 단계');
  log.info('npx tsx scripts/images/uploadGenesis2Images.ts');
}
