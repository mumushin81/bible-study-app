...
async function main() {
  ...
  if (!verse) {
    console.error('❌ 창세기 1:26을 찾을 수 없습니다');
    return;
  }
  ...
  words.forEach((word, idx) => {
    ...
    // "물고기" 관련 단어 강조
    if (word.korean.includes('물고기') || word.korean.includes('생선') ||
        word.korean.includes('다갓') || word.meaning.includes('물고기')) {
      console.log(`   ⭐ 물고기 관련 단어!`);
    }
    console.log('');
  });
