...
async function main() {
  ...
  if (!verses || verses.length === 0) {
    log.error('창세기 1:1-5 구절을 찾을 수 없습니다');
    return;
  }

  log.info(`찾은 구절: ${verses.map(v => v.reference).join(', ')}`);
  ...
  Array.from(byVerse.keys()).forEach(verseRef => {
    ...
    const withImage = verseWords.filter(w => w.current_image).length;
    log.info(`  ${verseRef}: ${verseWords.length}개 (기존 이미지: ${withImage}개)`);
  });
  ...
  // 결과 저장
  const outputDir = path.join(__dirname, '../../output/genesis_1_1to5_regenerate');
