...
async function main() {
  ...
  if (!verses || verses.length === 0) {
    log.error('창세기 1:2-5 구절을 찾을 수 없습니다');
    return;
  }
  ...
  if (!words || words.length === 0) {
    log.success('창세기 1:2-5의 모든 단어에 이미지가 있습니다!');
    return;
  }
  ...
  // 결과 저장
  const outputDir = path.join(__dirname, '../../output/genesis_1_2to5');
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  const resultPath = path.join(outputDir, `generation_${new Date().toISOString().split('T')[0]}.json`);
