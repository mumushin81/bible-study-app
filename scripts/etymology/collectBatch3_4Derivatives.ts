/**
 * Collect derivatives for Batch 3 & 4 roots from words table
 * Final batch: 19 remaining roots to complete 100%
 */
...
async function collectDerivatives() {
  console.log('🔍 Batch 3 & 4 파생어 수집 시작 (최종 19개 어근)\n');
  console.log('━'.repeat(80) + '\n');

  // 1. etymology_simple 데이터 로드
  const etymologyPath = path.join(__dirname, 'batch3_4_etymology_simple.json');
  const etymologyData = JSON.parse(fs.readFileSync(etymologyPath, 'utf-8'));

  console.log(`📂 ${etymologyData.total_roots}개 어근 데이터 로드 완료\n`);
  ...
  // 3. JSON 파일로 저장
  const outputPath = path.join(__dirname, 'batch3_4_complete_data.json');
  const outputData = {
    collected_at: new Date().toISOString().split('T')[0],
    batches: '3-4',
    roots_count: results.length,
    total_derivatives: results.reduce((sum, r) => sum + r.derivatives.length, 0),
    roots: results
  };
  ...
  console.log(`📖 총 ${outputData.total_derivatives}개 파생어`);
  console.log(`\n🎉 이제 42/42 어근 데이터 준비 완료! (100%)\n`);
}
