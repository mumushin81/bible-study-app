/**
 * Collect derivatives for Batch 1 roots from words table
 */
...
async function collectDerivatives() {
  console.log('🔍 Batch 1 파생어 수집 시작\n');
  console.log('━'.repeat(80) + '\n');

  // 1. etymology_simple 데이터 로드
  const etymologyPath = path.join(__dirname, 'batch1_etymology_simple.json');
  ...
  // 3. JSON 파일로 저장
  const outputPath = path.join(__dirname, 'batch1_complete_data.json');
  const outputData = {
